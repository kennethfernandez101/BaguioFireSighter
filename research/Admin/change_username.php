...
$id=$_GET['id'];
								...
								echo "No result found";
									...
									while($row = mysqli_fetch_array($sql)){
										...
										if(empty($_POST['new_username'])){
											echo "<style text/css>
											#new_username{border-color:#F00;}
											</style>";
											$errorMessage1="<span class='label label-danger'>Input Username</span>";
										}else{
											echo "<style text/css>
											#new_username{border-color:E1E1E1}
											</style>";
											$errorMessage1 = "";
										}
										if(empty($_POST['password'])){
											echo "<style text/css>
											#password{border-color:#F00;}
											</style>";
											$errorMessage2="<span class='label label-danger'>Input Password</span>";
										}else{
											echo "<style text/css>
											#password{border-color:E1E1E1}
											</style>";
											$errorMessage2 = "";
											$hashed_password = md5($_POST['password']);
												...
												if (($hashed_password == $row['password'])&&($_POST['old_username'] == $row['username'])){
													echo "<style text/css>
													#password{border-color:E1E1E1}
													</style>";
													$errorMessage2="";
															...
															mysqli_query( $con,"UPDATE admintbl SET username = '$username' WHERE staff_id ='$id'");
															
																					
							//getting the data of staff/Admin for documenting Log-Reports
			$staff_name = $staff_fname." ".$staff_lname;			
			$activity = "".$staff_name." Changed his/her Username";
			//Input a Log Event			
			...
			if (!mysqli_query($con,$log_event)){
				echo "Error Query insert_log_event";
											...
											window.alert("Successfully Updated. This Account will be Logged Out to Apply Changes");
 				...
 				<form class="form-horizontal" method="post">
                    ...
                    <div class="form-group">
      					<label for="username" class="col-sm-3 control-label">User Name:</label>
      						<div class="col-sm-9">
      							<input type="text" readonly class="form-control" id="old_username" name="old_username" aria-describedby="sizing-addon3" value="<?php echo $sql2['username'] ?>">
                                
                        	</div>
   					</div>
                    
                    <div class="form-group">
      					<label for="username" class="col-sm-3 control-label">New User Name:</label>
      						<div class="col-sm-9">
      							<input type="text" class="form-control" id="new_username" name="new_username" aria-describedby="sizing-addon3" value="<?php echo isset($_POST['new_username']) ? htmlspecialchars($_POST['new_username']) : ''; ?>">
                    ...
                    <div class="form-group">
      					<label for="username" class="col-sm-3 control-label">Password:</label>
      						<div class="col-sm-9">
      							<input type="password" class="form-control" id="password" name="password" input pattern=".{6,13}"  title="6 to 13 characters" value="">
