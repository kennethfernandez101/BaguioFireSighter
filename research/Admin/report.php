...
if (isset($_POST['submit'])){
	...
	if (trim(empty($_POST['origin_name']))){
		echo "<style text/css>
		#cause_id{border-color:#F00;}</style>";
	}else{
	$_SESSION['origin_name'] = $_POST['origin_name'];
		echo "<style text/css>
		#cause_id{border-color:transparent;}</style>";
	}
	...
	$date_alarm = new DateTime($_POST['date']);
	$date_out = new DateTIme($_POST['dfireout']);
	
	if ($date_alarm>=$date_out){
		echo "<style text/css>
		#dfireout_id{border-color:transparent;}</style>";
	}else{
		echo "<style text/css>
		#dfireout_id{border-color:#F00;}</style>";
	}
	
	if (($_POST['date']!="" && $_POST['timea']!="" && $_POST['hn']!="" && $_POST['strt']!="" && $_POST['brgy']!="" && $_POST['involve']!="" && $_POST['investigator']!="" && $_POST['casualty']!="" && $_POST['injured']!="" && $_POST['dfireout']!=""&& $_POST['fireout']!="" && $_POST['origin_name']!="" && $_POST['stat']!="")){
		
		
				
				$staff_name = $staff_fname." ".$staff_lname;
				
					$sql = "INSERT INTO recordstbl (dateandtime, time, house_no, street, barangay, involveestablishments, investigator, casualty, injured, dateoffireout,fireout,  causeoffire, statusofcaseammount) VALUES ('$_POST[date]', '$_POST[timea]', '$_POST[hn]', '$_POST[strt]', '$_POST[brgy]', '$_POST[involve]', '$_POST[investigator]', '$_POST[casualty]', '$_POST[injured]','$_POST[dfireout]', '$_POST[fireout]', '$_POST[origin_name]', '$_POST[stat]')";
...
<label for="lastname" class="col-sm-2 control-label">Date Of Alarm</label>
      						<div class="col-sm-6">
      							<input type="date" class="form-control" id="date_id" name="date" value="<?php echo isset($_POST['date']) ? htmlspecialchars($_POST['date']) : ''; ?>">
...
<label for="lastname" class="col-sm-2 control-label">Time Of alarm</label>
      						<div class="col-sm-6">
      							<input type="time" class="form-control" id="time_id" name="timea" value="<?php echo isset($_POST['timea']) ? htmlspecialchars($_POST['timea']) : ''; ?>">
...
<div class="form-group">
<label for="lastname" class="col-sm-2 control-label">Date of Fire Out</label>
      						<div class="col-sm-6">
      							<input type="date" class="form-control" id="dfireout_id" name="dfireout" value="<?php echo isset($_POST['dfireout']) ? htmlspecialchars($_POST['dfireout']) : ''; ?>">
