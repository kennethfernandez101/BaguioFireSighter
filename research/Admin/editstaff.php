...
if (isset($_POST['submit'])){
	...
	$_SESSION['rank'] = $_POST['rank'];
		...
		#date_id{border-color:transparent;}</style>";
				...
				mysqli_query( $con," UPDATE admintbl SET rank = '$_POST[rank]'  WHERE staff_id = '$id'");
				
				
				//getting the data of staff/Admin for documenting Log-Reports
			$staff_name = $staff_fname." ".$staff_lname;			
			$activity = "".$staff_name." Promoted a Fire Fighter";
			//Input a Log Event			
			...
			?>
					<script>
                    alert ("Successfully Promoted a Staff");
...
<h2 class="forms">Promote FireFighter</h2>
</div>
...
<label for="lastname" class="col-sm-2 control-label">Rank</label>
      						<div class="col-sm-6"> 
