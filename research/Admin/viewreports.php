...
if(isset($_POST['submit'])){
    ...
    <tbody>';
      ...
      echo '<tr>';
			...
			echo "<td><a href='editpage.php ?id=".$row['reportno']."' class='alert-link alert-info'>Edit</a>&nbsp;
				  </td>";
      echo'</tr>
    </tbody>';
	}
  echo'</table>';
		...
		$sql = mysqli_query($con,"SELECT * FROM recordstbl WHERE causeoffire like '%$_POST[origin]%' || dateandtime like '%$_POST[origin]%'");		
		if(!$sql){
			echo "No result found";
		}else{		
			 echo '<table class="table table-condensed table-striped table-bordered table-responsive">
...
<div class="alert alert-info col-sm-2" role="alert" style="padding:5px; margin-top:4px" align="center">
        <a href="report.php" class="alert-link">Back</a>
        </div>
        <br/>
