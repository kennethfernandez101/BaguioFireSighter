...
if(isset($_POST['submit'])){
	...
	if(!empty($_POST['compose'])){
		...
		//update Account
		mysqli_query($con,"UPDATE admintbl SET account = 'Inactive', reason = '$reason' WHERE staff_id='$id'");

											$staff_name = $staff_fname." ".$staff_lname;
											
											$activity = $staff_name." Deactivate Staff ID:".$id."";
			...
			if (!mysqli_query($con,$log_event)){
				echo "Error Query insert_log_event";
														...
														?>
				<script>
				window.alert('Account Deactivate <?php echo "Staff ID:".$id;?>');
